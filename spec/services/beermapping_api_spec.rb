...
describe "BeermappingApi" do
	...
	it "Empty table when HTTP GET does not return any place" do
		canned_answer = <<-END_OF_STRING
		...
		places = BeermappingApi.places_in("ccacsd")

		expect(places.size).to eq(0)
	end

	it "Table with multiple restaurants if HTTP GET return various places" do
		canned_answer = <<-END_OF_STRING
		...
		expect(place_last.name).to eq("Brewdog Helsinki")
		expect(place_last.street).to eq("Tarkk'ampujankatu 20")
	end
end